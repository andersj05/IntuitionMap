...
**Acquisition state:** no external or private dataset has been downloaded,
...
| LME-001 | [LongMemEval official repository](https://github.com/xiaowu0162/LongMemEval); proposed target is the cleaned release announced 2025-09 at [official HF](https://huggingface.co/datasets/xiaowu0162/longmemeval-cleaned), exact revision not yet pinned | `candidate-terms-review` | Temporal memory, multi-session reasoning, updates, evidence retrieval, abstention; preserve native QA. | C2 temporal-memory capability. | C3–C5; histories/questions are not the user’s latent thought links. | Repository `LICENSE` is MIT for software/documentation; the reviewed HF dataset page does not provide sufficiently clear, separately scoped dataset terms. | Source inspection and experiment planning only. | Do not download or redistribute until dataset-license scope is resolved and a revision is pinned. Never commit raw external data. | Artifact not retrieved; SHA-256 `—`. Source checked 2026-07-28. | Resolve data terms with the official release, pin revision/files, write experiment card, then acquire through a script into ignored `datasets/external/`. |
...
| LAMP-001 | [LaMP official repository](https://github.com/LaMP-Benchmark/LaMP), task and revision not yet selected | `candidate-choice` | Sparse personalization from user history; retain native classification/generation evaluation. | C2 personalization capability. | C3–C5; behavior/style proxies are not directed personal thought links. | LaMP code/data-creation methods are CC BY-NC-SA 4.0; official README states each underlying dataset retains its own terms. Avocado/LaMP-6 is not publicly available. | Compare fit against PersonalLLM; no data acquisition yet. | Never treat the umbrella license as covering source data. No raw external data in Git. | Artifact not retrieved; SHA-256 `—`. Source checked 2026-07-28. | Select a specific task only after recording its upstream source, version, terms, and commercial/redistribution restrictions. |
| PLLM-001 | [PersonalLLM official HF dataset](https://huggingface.co/datasets/namkoong-lab/PersonalLLM), exact commit not pinned | `candidate-choice` / `blocked` pending terms reconciliation | Cold-start preference adaptation across simulated personalities. | C2 personalization capability. | C3–C5; preferences are reward-model-generated and do not represent this user’s intuition. | Current HF metadata reports CC BY 4.0. The earlier canonical planning note recorded CC BY-NC 4.0. That conflict is unresolved and must be checked against a pinned card/repository revision. | Metadata inspection only. | No download, training, redistribution, or commit until the license discrepancy is resolved. | Artifact not retrieved; SHA-256 `—`. Source checked 2026-07-28. | Compare a pinned HF dataset card with the official repository/history and record the controlling terms before Phase 1 selection. |
...
| CN5-001 | [ConceptNet 5](https://conceptnet.io/), exact 5.x release not selected | `candidate-choice` | Typed generic commonsense relations and graph/relation sanity checks. | C2 generic relation handling. | C3–C5; aggregated commonsense cannot establish a user’s associations. | Official site states CC BY-SA 4.0. Individual source attribution and ShareAlike obligations must be preserved. | Compare fit against ATOMIC 2020; no data acquisition yet. | No raw dump in Git. Any distributed adapted database must satisfy attribution/ShareAlike obligations. | Artifact not retrieved; SHA-256 `—`. Source checked 2026-07-28. | Decide whether broad relation coverage is worth the licensing and source-mixture complexity; pin release and attribution plan. |
| ATOMIC-001 | [ATOMIC 2020 official repository](https://github.com/allenai/comet-atomic-2020), data archive/revision not pinned | `candidate-choice` | Directed if-then social, causal, and intent relation handling. | C2 generic directed-relation capability. | C3–C5; generic crowd knowledge is not personal intuition. | Official README: ATOMIC 2020 data CC BY; COMET code Apache 2.0. The exact data archive must carry matching terms when acquired. | Provisional relation-source preference because it is directed and narrower than ConceptNet; selection is not final. | Download only via a preparation script to ignored storage; commit only tiny fixtures if the controlling terms permit it. | Artifact not retrieved; SHA-256 `—`. Source checked 2026-07-28. | Write the Phase 1 experiment card, pin the archive, capture its license file and hash, then decide against ConceptNet. |
...
The provisional Phase 1 portfolio remains intentionally unresolved:
LongMemEval after its data terms are clarified, exactly one of LaMP or
PersonalLLM, and exactly one of ConceptNet or ATOMIC 2020. No public dataset is
required before Phase 0/G0 is complete.
